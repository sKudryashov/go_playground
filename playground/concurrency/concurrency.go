...
func SetBufferedWriter() {
	...
	for channelElement := range buffCh {
		fmt.Println("Channel data: Country - CHannel Element: ", channelElement.Country, channelElement.Text)
	}
...
func setTwoParallelWritersAndGetFirst(setOne, setTwo GreetingsSet) {
	...
	go func() {
		for _, item := range setOne {
			chan1 <- fmt.Sprintf("%s", item.Country)
		}
		close(chan1)
	}()
	go func() {
		for _, item := range setTwo {
			chan2 <- fmt.Sprintf("%s", item.Country)
		}
		close(chan2)
	}()
